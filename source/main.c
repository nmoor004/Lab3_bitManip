...
int main(void) {
	...
	unsigned char least_nibble = 0x00;
	unsigned char greatest_nibble = 0x00;
	unsigned char end_level_bits = 0x00;

	while (1) {
		least_nibble = PINA & 0x0F;
		greatest_nibble = PINA & 0xF0;
		
		if (least_nibble < 0x03) { //LVL1-2
			end_level_bits = 0x30; //PC5 +pc6 low level light
		}
		else if (least_nibble < 0x05) { //LVL3-4
			end_level_bits = 0x38; //PC5-PC4 +pc6 low level light
		}
		else if (least_nibble < 0x07) { //LVL5-6
			end_level_bits = 0x18; //PC5-PC3
		}
		else if (least_nibble < 0x0A) { //LVL7-9
			end_level_bits = 0x1C; //PC5-PC2
		}
		else if (least_nibble < 0x0D) { //LVL10-12
